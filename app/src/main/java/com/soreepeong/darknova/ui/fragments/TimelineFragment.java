...
public class TimelineFragment extends PageFragment implements SwipeRefreshLayout.OnRefreshListener, Handler.Callback, ImageCache.OnImageCacheReadyListener, TwitterEngine.TwitterStreamCallback {
	...
	public void onCreate(@Nullable Bundle savedInstanceState) {
		...
		try{
			if (savedInstanceState == null){
				mPage.mPageLastItemPosition = mPagePositionPreferences.getInt("position", 0);
				...
				mPage.mPageLastItemPosition = savedInstanceState.getInt("position", 0);
				mPage.mPageLastOffset = savedInstanceState.getInt("lastoffset", 0);
				mPage.mIsListAtTop = savedInstanceState.getBoolean("isattop", true);
			}
			mSelectedList.clear();
			if (savedInstanceState.getParcelableArrayList("selected-tweets") == null)
				return;
			for (Parcelable p : savedInstanceState.getParcelableArrayList("selected-tweets"))
				mSelectedList.add((Tweet) p);
		}catch(Exception e){
			e.printStackTrace();
		}
	}
	...
	public void onSaveInstanceState(Bundle outState) {
		...
		outState.putInt("position", mPage.mPageLastItemPosition);
		outState.putInt("lastoffset", mPage.mPageLastOffset);
		outState.putBoolean("isattop", mPage.mIsListAtTop);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mHandler.post(new Runnable() {
			...
			public void run() {
				...
				mViewProgress.setPadding(0, getResources().getDimensionPixelSize(R.dimen.abc_action_bar_default_height_material), 0, 0);
			}
		});
	...
	public void onStart() {
		super.onStart();
		TwitterStreamServiceReceiver.addStreamCallback(null, this);

		mImageCache = ImageCache.getCache(getActivity(), this);

		if (mIsPagePrepared && mBackgroundLoader != null) {
			mBackgroundLoader.onPostExecute(null);
		}
	}

	public void onActionBarShown(Toolbar actionBar) {
		mViewProgress.setPadding(0, getResources().getDimensionPixelSize(R.dimen.abc_action_bar_default_height_material), 0, 0);
	}

	public void onActionBarHidden(Toolbar actionBar) {
		mViewProgress.setPadding(0, 0, 0, 0);
	}
	...
	public void onDestroyView() {
		...
		mViewList.setLayoutManager(null);
	...
	private Object calculateListPositions() {
		...
		mPage.mPageLastItemPosition = mListLayout.findFirstVisibleItemPosition();
		while (mPage.mPageLastItemPosition > 0 && (mListLayout.findViewByPosition(mPage.mPageLastItemPosition - 1)) != null)
			mPage.mPageLastItemPosition--;
		View v = mListLayout.findViewByPosition(mPage.mPageLastItemPosition);
		mPage.mPageLastOffset = v != null ? v.getTop() - mViewList.getPaddingTop() : 0;
		return mAdapter.getItem(mPage.mPageLastItemPosition);
	}

	private void restorePositions(Object object) {
		if (!isPageReady())
			return;
	...
	private void saveListImmediately(){
		...
		if (!mIsPagePrepared || mList == null || mList.isEmpty())
			return;
		TwitterEngine.applyAccountInformationChanges(getActivity());
	...
	private void applyTweetChanges() {
		if (mPage.mListPending.isEmpty() || mViewFragmentRoot == null || mListApplier != null || mList == null)
			return;
		...
		synchronized (mPage.mListEditLock) {
			synchronized (mUpdaterLock) {
				...
				final List<Integer> mRemovedIndexes = new ArrayList<>();
				final List<Integer> mInsertedIndexes = new ArrayList<>();
				...
				for (int i = mTweetsToAdd.size() - 1; i >= 0 && !Thread.interrupted(); i--) {
					...
					if (index < 0) {
						...
						mInsertedIndexes.add(index);
					}
				}
				if (mPage.mIsListAtTop) {
					for (int i = newList.size() - 1; i >= MAX_MEMORY_HOLD; i--) {
						int oldIndex = previousList.indexOf(newList.remove(i));
						if (oldIndex == -1)
							continue;
						mRemovedIndexes.add(oldIndex);
					}
				}
				Collections.sort(mRemovedIndexes, Collections.reverseOrder());
				Collections.sort(mInsertedIndexes);
				final List<Tweet> applyingList = Collections.unmodifiableList(newList);
				mHandler.post(new Runnable() {
					@Override
					public void run() {
						synchronized (mUpdaterLock) {
							Object standard = calculateListPositions();
							mList = applyingList;
							mPage.mList = new WeakReference<>(mList);
							if (isPageReady()) {
								if (mQuickFilteredList == null) {
									if (mInsertedIndexes.size() + mRemovedIndexes.size() > 3) {
										mAdapter.notifyDataSetChanged();
									} else {
										for (Integer i : mRemovedIndexes)
											mAdapter.notifyNonHeaderItemRemoved(i);
										for (Integer i : mInsertedIndexes)
											mAdapter.notifyNonHeaderItemInserted(i);
									}
									restorePositions(standard);
									registerTimeUpdate();
									applyEmptyIndicator();
								}
	...
	public void setQuickFilter(String input) {
		...
		new FilterApplier(null, mList).execute(input, input);
	}
	...
	private static class OnCreateBackground extends AsyncTask<Object, Object, Object> {
		...
		protected void onPostExecute(Object o) {
			...
			View loadingView = mFragment.getView().findViewById(R.id.page_state);
			if (loadingView != null) {
			...
			for (Page.Element e : mPage.elements) {
				if (e.isHeaderView())
					mFragment.mAdapter.mElementHeaders.add(e.createHeader(R.layout.row_header_userbig));
				TwitterEngine engine = e.getTwitterEngine(mFragment.getActivity());
	...
	private class FilterApplier extends AsyncTask<String, Object, String> {
		...
		private FilteredTweet checkAndAdd(Tweet t) {
			...
			Matcher matchedUserId = null, matchedUserName = null, matchedText = null;
			...
			if (!(matchedText = mQuickFilterPattern.matcher(StringTools.unHanja(t.retweeted_status != null ? t.retweeted_status.text : t.text))).find())
				matchedText = null;
			...
			if ((userIdExists || userNameExists || emptyUser) && textExists) {
				...
				filtered.spannedText = SpannableStringBuilder.valueOf(t.retweeted_status != null ? t.retweeted_status.text : t.text);
				if (textExists)
				...
				try {
					switch (mElement.function) {
						case Page.Element.FUNCTION_HOME_TIMELINE:
							res = mElement.getTwitterEngine(getActivity()).getTweets("statuses/home_timeline", mLoadCount, since_id, max_id, false, true, PageRefresher.this);
							break;
						case Page.Element.FUNCTION_MENTIONS:
							res = mElement.getTwitterEngine(getActivity()).getTweets("statuses/mentions_timeline", mLoadCount, since_id, max_id, false, true, PageRefresher.this);
							break;
						case Page.Element.FUNCTION_SEARCH:
							res = mElement.getTwitterEngine(getActivity()).getSearchTweets(mElement.name, mLoadCount, since_id, max_id, false, true, PageRefresher.this);
							break;
						case Page.Element.FUNCTION_USER_TIMELINE:
							res = mElement.getTwitterEngine(getActivity()).getUserHome(mElement.id, mLoadCount, since_id, max_id, false, true, PageRefresher.this);
							break;
					}
				...
				if (v.equals(tweetActionRetweet)) {
					...
					for(int i = arrPreviews.length - 1; i >= 0; i--)
						if(arrPreviews[i].equals(v)){
							MediaPreviewActivity.previewTweetImages(getActivity(), null, t.retweeted_status == null ? t : t.retweeted_status, i);
						}
				}
			}
